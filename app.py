...
#--------------------------------------------------------------------------------------------------------------
        ...
        elif (len(half_data) > 0) and (len(data) == 0):
            df = half_df
            del half_df
            del half_data
        ...
        text_df.to_csv('{}.csv'.format(output_name), index=False)
        ...
        for n in range(count):
            ...
            fig.suptitle('MRN: {}, Name: {}, Date: {}, Time: {}, QRS VTI-3D: {}, QRS Dur: {}, QRS VTI-X: {}, QRS VTI-Y: {}, QRS VTI-Z: {}'.format(text_df.iloc[n, 0], text_df.iloc[n, 1], text_df.iloc[n, 4], text_df.iloc[n, 5], round((text_df.iloc[n, 14])/1000, 2), text_df.iloc[n, 9], round((text_df.iloc[n, 15])/1000, 2), round((text_df.iloc[n, 16])/1000, 2), round((text_df.iloc[n, 17])/1000, 2)))

            fig.savefig('{}_{}.png'.format(output_name, n))

        html_table = text_df.to_html()

        text_file = open("output.html", "w")
        text_file.write(html_table)
        for n in range(count):
            text_file.write("<img src='{}_{}.png' alt='Graph failed to load.' />".format(output_name, n))
        text_file.close()

        del df

    return render_template('index.html'), webbrowser.open_new_tab('output.html')